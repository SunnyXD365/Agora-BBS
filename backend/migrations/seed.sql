...
-- 插入示例主题帖
INSERT INTO topics (id, category_id, author_id, author_name, title, content, status, view_count, reply_count, created_at, updated_at)
VALUES 
    (1, 1, 1, 'admin', '欢迎来到 Agora-BBS 社区！', '这是一个基于 Go + Next.js + Temporal 架构构建的现代论坛系统。欢迎在此畅所欲言！', 'published', 102, 2, NOW(), NOW()),
    (2, 2, 2, 'alice', 'Go 1.23 特性解析与最佳实践', 'Go 1.23 带来了不少迭代器相关的增强，本文来聊聊如何在实际项目中使用 iterator...', 'published', 45, 0, NOW(), NOW()),
    (3, 3, 3, 'bob', '建议增加暗黑模式支持', '希望前端能支持 Dark Mode 切换，夜间看社区有点烫眼。', 'published', 12, 1, NOW(), NOW())
ON CONFLICT (id) DO NOTHING;

-- 插入示例回复
INSERT INTO posts (id, topic_id, author_id, author_name, parent_id, content, post_type, status, created_at, updated_at)
VALUES 
    (1, 1, 2, 'alice', NULL, '支持！期待后续更多 AI 功能落地。', 'reply', 'published', NOW(), NOW()),
    (2, 1, 3, 'bob', NULL, '环境一键拉起体验很好，赞！', 'reply', 'published', NOW(), NOW()),
    (3, 3, 1, 'admin', NULL, '收到建议，暗黑模式已列入后续路线图。', 'reply', 'published', NOW(), NOW())
ON CONFLICT (id) DO NOTHING;
