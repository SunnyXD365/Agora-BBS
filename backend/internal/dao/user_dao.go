...
func (d *UserDAO) GetUserByID(ctx context.Context, id int64) (*model.User, error) {
	query := `SELECT id, username, password_hash, email, avatar, created_at, updated_at FROM users WHERE id = $1`
	u := &model.User{}
	err := d.db.QueryRowContext(ctx, query, id).
		Scan(&u.ID, &u.Username, &u.PasswordHash, &u.Email, &u.Avatar, &u.CreatedAt, &u.UpdatedAt)
...
func (d *UserDAO) GetUserByUsername(ctx context.Context, username string) (*model.User, error) {
	query := `SELECT id, username, password_hash, email, avatar, created_at, updated_at FROM users WHERE username = $1`
	u := &model.User{}
