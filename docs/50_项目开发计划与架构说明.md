...
> 文档状态：实施基线。本文既是开发计划，也是新成员理解系统的入口；实现发生变化时必须同步更新。
...
| L3 | 参与盲审 | 7 天、有效阅读 300 分钟、5 次合规互动、信任分 ≥ 20 |

开发环境把累计阅读门槛缩短为 60/180/300 秒，并通过 Seed 提供各等级账号。隐藏信任分只向管理员展示；普通用户仅看到能力和下一阶段提示。
...
详细字段以 `docs/openapi.yaml` 为唯一公共契约；`docs/30_API文档.md` 提供中文调用说明。
...
每个里程碑只在 `dev-hx` 分支提交，完成自动验证后推送到 `origin/dev-hx`。
