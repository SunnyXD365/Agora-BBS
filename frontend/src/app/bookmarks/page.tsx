...
export default function BookmarksPage() {
  ...
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    bookmarkApi.list({ page, page_size: PAGE_SIZE }).then((result) => { if (!cancelled) { setItems(result.data.items); setTotal(result.data.total); } })
